...
class TraceContext {
  final String appPackage, screenName;
...
class SkillTraceRecorder {
  ...
  void recordStep(TraceStep step) {
    ...
    _currentTrace = ExecutionTrace(
        ...
        description: _currentTrace!.description,
        context: _currentTrace!.context,
        steps: [..._currentTrace!.steps, step],
        createdAt: _currentTrace!.createdAt);
  }

  ExecutionTrace? stopTrace() {
    final trace = _currentTrace;
    if (trace != null) _traces.add(trace);
    _currentTrace = null;
    return trace;
  }

  List<ExecutionTrace> getTraces() => List.unmodifiable(_traces);
  List<ExecutionTrace> findTraces(String name) =>
      _traces.where((t) => t.name.contains(name)).toList();

...
List<Tool> createSkillTools(SkillManager skillManager) => <Tool>[
      ...
      Tool(
        ...
        handler: (args) async {
          ...
          final shown = onlyEnabled
              ? all.where((e) => e['enabled'] == true).toList()
              : all;
          if (shown.isEmpty) {
          ...
          for (final s in shown) {
            ...
            sb.writeln(
                'enabled      : ${s['enabled']}  idempotent=${s['idempotent']}');
            if (s['dependencies'] is List &&
                (s['dependencies'] as List).isNotEmpty) {
              sb.writeln(
                  'dependencies : ${(s['dependencies'] as List).join(', ')} (请先全部启用)');
            }
            sb.writeln('registered   : ${s['registeredTools']} tools');
            final desc = (s['description'] as String).replaceAll('\n', ' ');
            sb.writeln(
                'description  : ${desc.length > kSkillDescPreviewMax ? '${desc.substring(0, kSkillDescPreviewMax)}…' : desc}');
          }
          sb.writeln(
              '-------------------------------------------------------------');
        ...
        schema: const {
          ...
          'properties': {
            'skill_id': {
              'type': 'string',
              'description':
                  'skill id，例如 "android_rpa" / "builtin_math_time" / "mcp_gateway"。支持同时启用多个：传 "id1,id2,id3" 用英文逗号分隔，我们会按数组顺序逐个启用（前一个失败则后一个仍尝试，结果里逐个列出）。',
            },
          },
          'required': ['skill_id'],
        },

...
class KnowledgeRagSkill extends Skill {
  ...
  @override
  String get description => '搜索本地知识库（.txt 文档），返回与查询最相关的前 3 个文本片段。纯词频匹配、无 embedding 依赖、离线可用。用户提问中提到"查知识库"/"查资料"/"从文档里找"时你可以先启用它。';
...
class AgentLongTermMemorySkill extends Skill {
  ...
  @override
  String get description => '跨会话长期 KV 记忆（本地文件持久化）。操作：op=set/get/delete/list/clear_prefix。⚠ 代码层不做任何自动记笔记/自动摘要，全靠你 (LLM) 主动决定存什么。用户提到"记住我的手机号/姓名/常用地址/常用按钮坐标/下次要…"这类需要跨 session 保留的信息时启用。';
...
class ExecutePlanSkill extends Skill {
  ...
  final Future<ToolResult> Function(String name, Map<String, dynamic> args) executor;
  ...
  @override
  String get description => '多步编排工具：你 (LLM) 写 steps[] 数组 (最多 50 步)，代码 100% 按你写的顺序严格执行，不做任何修改/跳过/重试。配合 stop_on / step.expect_ok / stop_if_contains 自主决定流程控制。注：你想省 N 轮端侧推理时启用；如果只有 1~2 步直接调工具更快。';
...
class VisionAnalyzeSkill extends Skill {
  ...
  final Future<String> Function(String imagePath, String question)? visionAnalyze;
  ...
  @override
  String get description => '把任意本地图片路径丢给多模态 VLM 模型分析，可以问 OCR 内容、这张图里有什么、某区域的大致坐标、UI 里某个按钮在哪等。⚠ 只做分析不做操作；要操作 Android UI 请同时启用 android_rpa。用户纯咨询图片内容时启用即可。';
  ...
  Future<void> onEnable(void Function(Tool) registerTool) async {
    ...
    if (va == null) {
      registerTool(Tool(
        name: 'vision_analyze',
        description: '【占位】VLM 回调未初始化，实际不可用。先启用 android_rpa 或用正确的 visionAnalyze 初始化 skill_manager。',
        schema: const {'type': 'object', 'properties': {}},
        handler: (_) async => const ToolResult.error('vision_analyze: VLM callback not initialized. Try enabling android_rpa instead.'),
      ));
      return;
    }
    registerTool(Tool(
      name: 'vision_analyze',
      description: '【VLM 视觉分析 · 独立】传入本地图片路径 image_path  + 你要问的问题 prompt，Omni 多模态模型会基于整张图回答。可用于：OCR 文字、描述图片内容、"这个按钮的坐标在哪"、"第几条朋友圈是广告" 等纯视觉问题。如需实际点击请同时启 android_rpa。',
      schema: const {
        'type': 'object',
        'properties': {
          'image_path': {'type': 'string', 'description': '本地图片文件绝对路径（如 /sdcard/Pictures/x.png），不支持网络 URL。'},
          'prompt': {'type': 'string', 'description': '你想问这张图的问题。如 "帮我把图里所有文字按行转录出来" 或 "绿色点赞按钮的中心坐标是？(用 x,y 百分比)"'},
        },
      ...
      handler: (args) async {
        ...
        if (p.isEmpty || q.isEmpty) return const ToolResult.error('image_path / prompt 不能为空');
        try {
...
List<Tool> createSkillRegisterJsonTools({
  ...
  out.add(Tool(
    ...
    handler: (args) async {
      ...
      return ToolResult.ok('OK: registered JSON skill id=$id (${newSkill.name} v${newSkill.version}, $n tools, deps=${newSkill.dependencies}). Use skill_enable $id to enable.');
    },
  ));

  out.add(Tool(
    name: 'skill_register_json_example',
    description: '输出 3 个典型 JSON skill 示例（callMcp / callTool 参数重映射 / template），直接复制改几个字段就能用 skill_register_json 注册。',
    schema: const {'type': 'object', 'properties': {}},
    handler: (_) async {
      const ex1 = '''
...
Tool createSkillFromTraceTool(SkillManager skillManager) {
  return Tool(
    ...
    description: '【从工具调用轨迹创建可复用 Skill】把你刚刚执行过的一系列工具调用保存为一个可复用的新 Skill，之后 skill_enable 就能一键重放。'
    ...
    schema: const {
      ...
      'properties': {
        'skill_name': {
          'type': 'string',
          'description': '新 Skill 的唯一 ID（也是名字，简短如 wechat_auto_like、daily_report）。',
        },
        'skill_description': {
          'type': 'string',
        ...
        'steps': {
          ...
          'items': {
            'type': 'object',
            'properties': {
              'tool': {'type': 'string', 'description': '工具名，如 android_click_by_text、web_search 等。'},
              'args': {'type': 'object', 'description': '传给该工具的参数，key-value 对。支持 {args.xxx} 模板占位符，让重用时能传不同参数。'},
            },
            'required': ['tool', 'args'],
          },
        },
    ...
    handler: (args) async {
      ...
      try {
        ...
        return ToolResult.error('Skill 已注册但启用失败：$e\n$st。可用 skill_enable $name 手动启用。');
      }

...
class _MemoryValue {
  final String value;
  final DateTime createdAt, lastAccessAt;
  ...
  _MemoryValue copyWith({DateTime? lastAccessAt}) => _MemoryValue(
      value: value,
      createdAt: createdAt,
...
class MemoryEntry {
  final String key;
  final String value;
  final DateTime createdAt, lastAccessedAt;
  final int accessCount;
  const MemoryEntry(
      {required this.key,
      required this.value,
...
class VectorMemoryBackend {
  VectorMemoryBackend(
      {required String storagePath, EmbeddingService? embeddingService})
  ...
  Future<String?> get(String key) async {
    ...
    if (_store.containsKey(key)) {
      _store[key] = _store[key]!.copyWith(lastAccessAt: DateTime.now());
  ...
  Future<void> set(String key, String value) async {
    await initialize();
    _store[key] = _MemoryValue(
        value: value,
        createdAt: DateTime.now(),
        lastAccessAt: DateTime.now(),
        accessCount: (_store[key]?.accessCount ?? 0) + 1);
    await _saveToFile();
  }
  ...
  Future<List<({String key, String value, DateTime mtime})>> list(
    ...
    return _store.entries
        ...
        .map((e) =>
            (key: e.key, value: e.value.value, mtime: e.value.lastAccessAt))
        .toList()
      ..sort((a, b) => b.mtime.compareTo(a.mtime))
      ..take(limit);
  }

  Future<List<({String key, String value, double score})>> semanticSearch(
      String query,
      {int topK = 5}) async {
    await initialize();
    ...
    for (final entry in _store.entries) {
      ...
      final similarity =
          EmbeddingService.cosineSimilarity(queryEmbedding, valueEmbedding);
      scored.add(_ScoredEntry(entry.key, entry.value.value, similarity));
    }
    scored.sort((a, b) => b.score.compareTo(a.score));
    return scored
        .take(topK)
        .map((s) => (key: s.key, value: s.value, score: s.score))
        .toList();
  }
  ...
  Future<void> _loadFromFile() async {
    final file = File('$_storagePath/vector_memory.json');
    ...
    try {
      ...
      for (final entry in data.entries) {
        ...
        _store[entry.key] = _MemoryValue(
            value: v['value'] as String? ?? '',
            createdAt: DateTime.tryParse(v['created_at'] as String? ?? '') ??
                DateTime.now(),
            lastAccessAt:
                DateTime.tryParse(v['last_access_at'] as String? ?? '') ??
                    DateTime.now(),
            accessCount: v['access_count'] as int? ?? 0);
      }
    } catch (_) {}
  }
  ...
  Future<void> _saveToFile() async {
    ...
    for (final entry in _store.entries) {
      data[entry.key] = {
        ...
        'last_access_at': entry.value.lastAccessAt.toIso8601String(),
        'access_count': entry.value.accessCount
      };
    }
    await File('$_storagePath/vector_memory.json')
...
class _ScoredEntry {
  ...
  final String value;
  final double score;
  _ScoredEntry(this.key, this.value, this.score);
}

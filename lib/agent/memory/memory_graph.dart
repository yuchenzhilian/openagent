...
class MemoryGraph {
  ...
  void removeNode(String key) {
    _nodes.remove(key);
    _edges.removeWhere((e) => e.sourceKey == key || e.targetKey == key);
  }

  void addEdge(MemoryEdge edge) {
    if (!_nodes.containsKey(edge.sourceKey) ||
        !_nodes.containsKey(edge.targetKey)) return;
    _edges.add(edge);
  }
  ...
  void addTemporalEdges({int windowMinutes = 5}) {
    ...
    for (int i = 0; i < entries.length; i++) {
      for (int j = i + 1; j < entries.length; j++) {
        final diff = entries[i]
            .timestamp
            .difference(entries[j].timestamp)
            .inMinutes
            .abs();
        if (diff <= windowMinutes) {
          addEdge(MemoryEdge(
              sourceKey: entries[i].key,
              targetKey: entries[j].key,
              type: EdgeType.temporal,
              weight: 1.0 - (diff / windowMinutes)));
        }
      }
    }
  }

  List<MemoryNode> findRelated(String query,
      {int maxDepth = 2, int maxResults = 10}) {
    final visited = <String>{};
    final results = <MemoryNode>[];
    if (_nodes.containsKey(query)) {
      _traverse(query, visited, results,
          maxDepth: maxDepth, maxResults: maxResults);
      return results;
    }
    final prefixMatches = _nodes.keys.where((k) => k.contains(query)).toList();
    for (final key in prefixMatches) {
      _traverse(key, visited, results,
          maxDepth: maxDepth, maxResults: maxResults);
  ...
  void _traverse(String key, Set<String> visited, List<MemoryNode> results,
      {required int maxDepth, required int maxResults, int depth = 0}) {
    if (depth > maxDepth ||
        results.length >= maxResults ||
        visited.contains(key)) return;
    visited.add(key);
    final node = _nodes[key];
    if (node == null) return;
    results.add(node);
    for (final edge in _edges.where((e) => e.sourceKey == key)) {
      _traverse(edge.targetKey, visited, results,
          maxDepth: maxDepth, maxResults: maxResults, depth: depth + 1);
    }
    for (final edge in _edges.where((e) => e.targetKey == key)) {
      _traverse(edge.sourceKey, visited, results,
          maxDepth: maxDepth, maxResults: maxResults, depth: depth + 1);
    }
  }
  ...
  factory MemoryGraph.fromJson(Map<String, dynamic> j) {
    ...
    for (final n in (j['nodes'] as List? ?? [])) {
      if (n is Map<String, dynamic>) graph.addNode(MemoryNode.fromJson(n));
    }
    for (final e in (j['edges'] as List? ?? [])) {
      if (e is Map<String, dynamic>)
        graph._edges.add(MemoryEdge(
            sourceKey: e['source_key'] as String? ?? '',
            targetKey: e['target_key'] as String? ?? '',
            type: EdgeType.values.firstWhere((t) => t.name == e['type'],
                orElse: () => EdgeType.reference),
  ...
  static Future<MemoryGraph> loadFromFile(String path) async {
    ...
    if (!await file.exists()) return MemoryGraph();

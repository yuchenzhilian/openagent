...
class MemoryScorerConfig {
  ...
  const MemoryScorerConfig({
    this.frequencyWeight = 0.3, this.recencyWeight = 0.4, this.semanticWeight = 0.3,
...
class MemoryEntry {
  ...
  const MemoryEntry({
    ...
    required this.lastAccessedAt, required this.accessCount,
  });
}

class MemoryScorer {
  MemoryScorer({MemoryScorerConfig? config}) : _config = config ?? MemoryScorerConfig();
  ...
  double _recencyScore(MemoryEntry entry) {
    final hoursSinceAccess = DateTime.now().difference(entry.lastAccessedAt).inHours;
  ...
  double _semanticScore(MemoryEntry entry) {
    ...
    double score = 0.0;
    if (keyLower.startsWith('user:') || keyLower.startsWith('prefs:')) score += 0.3;
    if (keyLower.startsWith('learned_ui:')) score += 0.2;
    final importantKeywords = ['password', 'token', 'secret', 'account', '密码', '账号'];
    for (final kw in importantKeywords) { if (lower.contains(kw)) { score += 0.1; break; } }
    if (entry.value.length > 500) score += 0.1;
    return min(score, 1.0);
  }

  List<({MemoryEntry entry, double score})> scoreBatch(List<MemoryEntry> entries) {

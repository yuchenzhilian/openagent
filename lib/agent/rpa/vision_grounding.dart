...
class VisionGrounding {
  Future<GroundingResult> ground(String screenshotPath, String description,
      {Future<String> Function(String imagePath, String prompt)?
          vlmAnalyze}) async {
    if (vlmAnalyze == null)
      return GroundingResult(
          description: description, confidence: 0.0, found: false);
    final prompt =
        '在屏幕截图中找到"$description"的位置。请返回该元素在屏幕中的相对坐标 (x,y,w,h)，取值范围 0.0-1.0。格式: {x:0.5, y:0.5, w:0.2, h:0.1}';
  ...
  GroundingResult _parseVlmResponse(String response, String description) {
    try {
      ...
      if (jsonStart >= 0 && jsonEnd > jsonStart) {
        ...
        final x = _extractValue(json, 'x');
        final y = _extractValue(json, 'y');
        final w = _extractValue(json, 'w');
        final h = _extractValue(json, 'h');
        if (x != null && y != null)
          return GroundingResult(
              ...
              region: ScreenRegion(
                  x: x.clamp(0.0, 1.0),
                  y: y.clamp(0.0, 1.0),
                  width: w?.clamp(0.0, 1.0) ?? 0.1,
                  height: h?.clamp(0.0, 1.0) ?? 0.1),
              confidence: 0.8,
              found: true);
      }
    } catch (_) {}
  ...
  ({double x, double y}) centerOf(ScreenRegion region) =>

...
Tool _antiDetectionCheckTool(AndroidAutomationService s) => Tool(
      ...
      handler: (args) async {
        ...
        if (isBanking || isPayment) {
          sb.writeln('⚠ 建议：避免使用无障碍服务操作此应用，优先使用 Shizuku 或调用 android_anti_detection_safe_mode enabled=true');
        }
...
Tool _keepAliveTool(AndroidAutomationService s) => Tool(
      ...
      description:
          '【安全】将本应用添加到系统省电白名单、防清理列表，并启动前台服务保活，避免后台被系统杀掉。'
          '需要 Shizuku 已授权。执行后 Agent 可在后台持续运行不被系统清理。',
      schema: _props({
        'action': {
          'type': 'string',
          'enum': [
            'add_whitelist', 'check_status', 'remove_whitelist',
            'start_foreground', 'stop_foreground', 'is_running',
          ],
      ...
      handler: (args) async {
        ...
        if (action == 'is_running') {
          final r1 = await s.gshell('dumpsys activity services $fgService 2>/dev/null');
        ...
        if (action == 'start_foreground') {
          ...
          return ToolResult.ok(r1.ok ? '✅ 前台服务已启动' : '⚠ 启动前台服务失败（可能需要 Shizuku）');
        }
        ...
        if (action == 'check_status') {
          ...
          final r2 = await s.gshell('dumpsys activity services $fgService 2>/dev/null');
          final sb = StringBuffer();
          sb.writeln('===== 保活状态 =====');
          sb.writeln('省电白名单: ${r1.stdout.contains(pkg) ? "✅ 已加入" : "❌ 未加入"}');
          sb.writeln('前台服务: ${r2.stdout.contains(fgService) ? "✅ 运行中" : "⚠ 未运行"}');
          // 额外检查电源管理可见性
          final r3 = await s.gshell('dumpsys power | grep $pkg 2>/dev/null');
          sb.writeln('电源管理可见性: ${r3.stdout.contains(pkg) ? "✅ 可见" : "⚠ 可能不可见"}');
          return ToolResult.ok(sb.toString());
        }
        ...
        if (action == 'remove_whitelist') {
          ...
          await s.gshell('am stopservice -n $pkg/.automation.$fgService 2>/dev/null');

...
class VectorIndex {
  ...
  Future<void> addChunk(IndexedChunk chunk) async {
    await initialize();
    _chunks.add(chunk);
  }

  Future<void> addChunks(List<IndexedChunk> chunks) async {
    for (final c in chunks) await addChunk(c);
  }

  Future<List<SearchResult>> search(List<double> queryEmbedding,
      {int topK = 5}) async {
    await initialize();

...
void main() {
  ...
  group('Intent Classification Accuracy', () {
    ...
    test('math expressions (10 cases)', () {
      final cases = ['2+2', '3*5', 'sqrt(16)', 'calculate 10% of 50', 'what is 2^8',
                     '100 / 4', 'sin(30)', 'log(100)', 'abs(-5)', '25.5 * 3'];
    ...
    test('date/time queries (10 cases)', () {
      final cases = ['what time is it', 'current date', 'today', '几点了', '今天星期几',
                     'now', 'what day is it', '现在几点', '日期', 'time now'];
      for (final c in cases) {
    ...
    test('Android automation (10 cases)', () {
      final cases = ['打开微信', 'click the settings button', '发送短信给张三',
                     '打开抖音', 'swipe down', '拍照', '截图', '关闭蓝牙',
                     'open app com.test', 'scroll to bottom'];
      for (final c in cases) {
        expect(classifier.classify(c).category, IntentCategory.androidAutomation,
            reason: 'Failed for: $c');
      }
    });

    test('general chat (10 cases)', () {
      final cases = ['Hello', 'How are you?', '你好', '谢谢', '再见',
                     'Nice to meet you', 'What do you think?', '好的', '是的', '确实如此'];
      for (final c in cases) {
  ...
  group('ToolValidator Schema Validation', () {
    ...
    test('required field detection', () {
      ...
      expect(validator.validate(tool, {'name': 'a', 'email': 'b'}).isValid, isTrue);
    ...
    test('type coercion', () {
      ...
      expect(validator.validate(tool, {'count': 5, 'ratio': 0.5, 'flag': true}).isValid, isTrue);

...
void main() {
  group('ConstraintDecoder', () {
    test('passes through free text', () async {
      final output =
          await _runDecoder(ConstraintDecoder(), ['hello', ' world', ' 你好']);
      expect(output, 'hello world 你好');
    });
    test('validates JSON in tool call', () async {
      final output = await _runDecoder(ConstraintDecoder(), [
        'text ',
        '<tool_call>',
        '{"name":"calc","args":{"x":1}}',
        '</tool_call>'
  ...
  group('IntentClassifier', () {
    ...
    test(
        'math',
        () => expect(
            classifier.classify('2+2').category, IntentCategory.mathCalc));
    test(
        'date',
        () => expect(classifier.classify('what time is it').category,
            IntentCategory.dateTime));
    test(
        'android',
        () => expect(classifier.classify('打开微信').category,
            IntentCategory.androidAutomation));
    test(
        'knowledge',
        () => expect(classifier.classify('what is ML').category,
            IntentCategory.knowledgeQuery));
    test(
        'web',
        () => expect(classifier.classify('search for Flutter').category,
            IntentCategory.webSearch));
    test(
        'chat',
        () => expect(
            classifier.classify('Hello').category, IntentCategory.generalChat));
  });
